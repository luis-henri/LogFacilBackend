...
export const importFromTxt = async (req: Request, res: Response): Promise<void> => {
    ...
    const situacaoQuery = req.query.situacao as string | undefined;
    ...
    try {
        ...
        const records: string[][] = parse(conteudoFicheiro, {
            delimiter: ';',
        ...
        let requisicaoAtual: Partial<ParsedRequisicao> = { itens: [] };
        
        // Captura a data global do arquivo (primeira ocorrência com hora completa)
        let dataGlobalArquivo: Date | null = null;
        ...
        function tryParseItemFromRow(row: string[]): { endereco?: string; quantidade: number; unidade: string; descricao: string } | null {
        ...
        const requisicoesExistentes = await prisma.requisicao.findMany({
